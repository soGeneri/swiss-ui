...
export interface RichTextEditorProps {
  ...
  /** Placeholder text shown when editor is empty (default: "Enter text...") */
  placeholder?: string;
  /** Additional CSS classes for the container */
  className?: string;
  /** Minimum height of the editor (default: "60px") */
  minHeight?: string;
  /** i18n labels for LinkDialog */
  linkDialogLabels?: {
    displayText?: string;
    url?: string;
    cancel?: string;
 ...
 * and @tiptap/extension-underline as peer dependencies.
 */
    ...
    editorProps: {
      attributes: {
        class: cn(
          ...
          'prose-strong:font-bold prose-em:italic prose-a:text-[var(--swiss-primary,#1D4ED8)] prose-a:underline'
        ),
  ...
  if (!isMounted) {
    return (
      <div className={cn('space-y-1', className)}>
        <div className="flex items-center gap-1 p-1 border border-[var(--swiss-border,#000000)] bg-[var(--swiss-panel,#E5E5E0)] h-9" />
        <div
          className={cn(
            'w-full border border-[var(--swiss-border,#000000)] bg-white',
            'px-3 py-2 text-sm text-[var(--swiss-placeholder,#9CA3AF)] rounded-none'
          )}
          style={{ minHeight }}
        >
          {placeholder}
  ...
  return (
    <div className={cn('space-y-1', className)}>
      ...
      <div
        className={cn(
          'w-full border border-[var(--swiss-border,#000000)] bg-white',
          'px-3 py-2 text-sm text-[var(--swiss-ink,#000000)] rounded-none',
          'focus-within:ring-1 focus-within:ring-[var(--swiss-focus-ring,#1D4ED8)]',
          '[&_.ProseMirror]:outline-none [&_.ProseMirror]:min-h-[36px]',
          '[&_.ProseMirror_p]:m-0',
          '[&_.ProseMirror_a]:text-[var(--swiss-primary,#1D4ED8)] [&_.ProseMirror_a]:underline'
        )}
        style={{ minHeight }}
      >
      ...
      {showLinkDialog && (
        <LinkDialog
          ...
          displayTextLabel={linkDialogLabels.displayText}
          urlLabel={linkDialogLabels.url}
          cancelLabel={linkDialogLabels.cancel}
          removeLinkLabel={linkDialogLabels.removeLink}
          addLinkLabel={linkDialogLabels.addLink}
          updateLinkLabel={linkDialogLabels.updateLink}
          addLinkTitle={linkDialogLabels.addLinkTitle}
          editLinkTitle={linkDialogLabels.editLinkTitle}
        />
      )}
    </div>
  );
